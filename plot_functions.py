...
def calc_RMS(RMS1,RMS2,RMS3):
	for j in normalized_spectra:
		...
		p=np.where(chi==np.min(chi))[0]
 		a=np.roll(sinterp,p)
 		diff=a-s1interp
 		rms1=np.around((np.mean(diff[1:1000]**2))**(1/2.),decimals=7)
		rms2=np.around((np.mean(diff[1000:2000]**2))**(1/2.),decimals=7)
		rms3=np.around((np.mean(diff[2000:3500]**2))**(1/2.),decimals=7)
		RMS1.append(rms1)
		RMS2.append(rms2)
		RMS3.append(rms3)
